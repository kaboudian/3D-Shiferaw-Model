...
void main() {
    ...
    int  voxelId = gl_InstanceID ;

    ivec2 isize = textureSize(compressed3dCrdt,        0 ) ;
    ivec2 voxelIndex = ivec2( voxelId % isize.x , voxelId / isize.x ) ;
    vec3 pos =  
        (texelFetch(compressed3dCrdt , voxelIndex, 0 ).xyz - vec3(0.5))*2. ; 
    pos += voxelSize*0.005*2.*(vertCrds[vertId]-vec3(0.5)) ; 

    vec4  color   = texelFetch(  icolor, voxelIndex, 0 ) ; 
   
   // vec4 materialColor = color.r < 0.5 ? 
   //     vec4(vec3(0.8),1.) : texture( clrm, vec2(0.5,color.r/3.));//vec4(0.1,0.1,(color.r+1.)/4.5,1.) ;

    vec4 materialColor = texture( clrm, vec2(color.r/3.,0.5));
